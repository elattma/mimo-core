...
class Weaver:
    ...
    def minify(self, request_embedding: List[float], blocks: List[Block], token_limit: int, encoding_name: str = 'cl100k_base', num_blocks: int = None) -> None:
        ...
        # count tokens
        block_to_tokens_map: Dict[Block, int] = {}
        total_token_count = 0
        for block in blocks:
            token_count = self._count_tokens_block(block, encoding_name)
            block_to_tokens_map[block] = token_count
        ...
        if num_blocks is not None:
            ...
            total_token_count = sum(block_to_tokens_map[block] for block in blocks)
            return

        # pop blocks until total token count is below limit besides the last block
        while len(blocks) > 1:
            last_block = blocks[-1]
            last_block_token_count = block_to_tokens_map[last_block]
        ...
        if total_token_count > token_limit:
            ...
            chunk_to_property: Dict[Chunk, UnstructuredProperty] = {}
            chunk_to_tokens: Dict[Chunk, int] = {}

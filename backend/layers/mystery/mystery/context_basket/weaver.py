...
class BasketWeaver:
    ...
    @staticmethod
    def minify_context_basket(request: Request, context_basket: ContextBasket, limit_tokens: int, encoder_name: str) -> None:
        ...
        while remaining_tokens > 0 and context_counter < contexts_len - 1:
            ...
            context_tokens = count_tokens(context, encoder_name)
...
def sort_contexts(focal_embedding: List[float], contexts: List[Context]) -> None:
    ...
    for context in contexts:
        sort_list_embeddings(focal_embedding, context.blocks, [block.embedding for block in context.blocks])

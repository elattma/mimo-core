...
def handler(event: dict, context):
    ...
    fetch_responses: List[FetchResponse] = []
    futures = None
    ...
    if futures:
        for future in as_completed(futures):
            ...
            if response_item:
                fetch_responses.append(response_item)
...
def discover_fetch_embed_load(user: str, fetcher: Fetcher, db: GraphDB, pinecone: Pinecone, ner: NER, openai_api_key: str) -> bool:
    # TODO: add pagination
    ...
    pinecone_response = pinecone.upsert(documents=documents, user=user)
    response = db.create_documents(documents)
